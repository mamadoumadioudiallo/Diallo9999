...
// Fichier : superviseur/profil.php
// Rôle    : Profil du compte superviseur — changement de mot de passe
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'changer_mdp') {
    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!password_verify($ancienMdp, $hashActuel)) {
            ...
            $stmt->execute([$nouveauHash, $idUser]);

            enregistrerAudit($pdo, 'changement_mdp', 'Compte superviseur');
...
<div style="display:grid;grid-template-columns:240px 1fr;gap:20px;max-width:760px;margin:0 auto;">
    ...
    <div class="panel" style="padding:24px;text-align:center;">
        ...
        <?php endif; ?>
        <h3 style="margin-bottom:6px;font-size:16px;"><?= htmlspecialchars($utilisateur['prenom'] . ' ' . $utilisateur['nom']) ?></h3>
        <span class="badge badge-success">Superviseur</span>
        <p style="font-size:12px;color:#999;margin-top:14px;word-break:break-word;"><?= htmlspecialchars($utilisateur['email']) ?></p>
    </div>
